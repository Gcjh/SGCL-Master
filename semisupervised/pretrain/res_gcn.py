...
class ResGCN(torch.nn.Module):
    ...
    def forward_cl(self, data, node_imp):
        x, edge_index, batch = data.x, data.edge_index, data.batch
        ...
        return self.forward_BNConvReLU_cl(x, edge_index, batch, node_imp, xg)

    def forward_BNConvReLU_cl(self, x, edge_index, batch, node_imp, xg=None):
        x = self.bn_feat(x)
        x = F.relu(self.conv_feat(x, edge_index))
        for i, conv in enumerate(self.convs):
            x_ = self.bns_conv[i](x)
    ...
    def loss_ra(self, x1, x2, x3, temp=0.2, lamda=0.01):
        batch_size, _ = x1.size()
        x1_abs = x1.norm(dim=1)
        x2_abs = x2.norm(dim=1)
