...
from augmentation import drop_nodes_prob, drop_nodes_cp, permute_edges_prob, permute_edges_cp
...
class TUDataset_aug(InMemoryDataset):
    ...
    def get(self, idx):
        ...
        if self.aug == 'L':
            ...
            aug_data = drop_nodes_prob(deepcopy(data), data.node_score, self.rho)
            cp_data= drop_nodes_cp(deepcopy(data), data.node_score, self.rho)  ####

            return data, aug_data, cp_data
...
def degree_matrix(data):

    edge_index = data.edge_index.numpy()
    d = Counter(edge_index[0])
    d = list(d.items())
    d = torch.tensor(d)
    d = d[:, -1]
    return d
...
if __name__ == '__main__':
    ...
    dataset = TUDataset_aug(path, name=DS, aug="none", rho=0.9)
    ...
    f_1 = open('node.txt', "w")
    f_2 = open('graph.txt', "w")
    for step, data in enumerate(dataloader):
        #data = permute_edge(data[0], 0, 5)
        data = data[0]
        print(data)
        '''for i in range(data.num_nodes):
            print(i, 1, file=f_1)
        for i in range(data.edge_index.shape[1]):
            print(int(data.edge_index[0][i]), int(data.edge_index[1][i]), file=f_2)'''
        break
