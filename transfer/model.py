import torch
from torch_geometric.nn import MessagePassing, GlobalAttention, Set2Set
...
class GNN_graphpred(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, num_tasks, JK = "last", drop_ratio = 0, graph_pooling = "mean", gnn_type = "gin"):
        ...
        self.node_imp_estimator = GNN_Explainer(num_layer=3, emb_dim=emb_dim, JK=JK, drop_ratio=0.0, L=False)
...
class GNN_Explainer(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0, L=False):
    ...
    def forward(self, *argv):
        if len(argv) == 4:
            ...
            data = argv[0]
            x, edge_index, edge_attr, degree = data.x, data.edge_index, data.edge_attr, data.degree
        else:
            raise ValueError("unmatched number of arguments.")
        ...
        if self.L == True:
            node_representation = node_representation.div(degree + 1)
            return node_representation
...
class Explainer(torch.nn.Module):
    def __init__(self, device, num_features, dim, num_gc_layers, L = False):
        ...
        for i in range(num_gc_layers):
            ...
            if L == True:
                if i and i < num_gc_layers - 1:
                    ...
                    nn = Sequential(Linear(dim, dim), ReLU(), Linear(dim, 2))
    ...
    def forward(self, data, aug = False):
        ...
        if aug == True:
            index = data.L_index
            L_num = index.shape[0]
            x = x.repeat(L_num, 1, 1).to(device)
            mask = torch.ones(x.shape[0], x.shape[1], 1).to(device)
            for i in range(L_num):
                mask[i][index[i]] = 0
            
            for i in range(self.num_gc_layers):
                if i !=  self.num_gc_layers - 1:
                    xx = torch.zeros(x.shape[0], x.shape[1], self.dim).to(device)
                    for j in range(L_num):
                        xx[j] = F.relu(self.bns[i](self.convs[i](mask[j] * x[j], edge_index)))
                    x = xx
                else:
                    xx = torch.zeros(x.shape[0], x.shape[1], self.fc_dim).to(device)
                    for j in range(L_num):
                        xx[j] = self.bns[i](self.convs[i](mask[j] * x[j], edge_index))
                    x = xx

                f_x = x.reshape(-1, x.shape[-1])
                f_mask = mask.reshape(-1, 1)      
                xs.append(f_mask * f_x)
        else:
            for i in range(self.num_gc_layers):
                if i !=  self.num_gc_layers - 1:
