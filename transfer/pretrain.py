...
class graphcl(nn.Module):
    ...
    def forward_cl(self, x, edge_index, edge_attr, batch, node_imp):
...
def train(args, model, gen, device, dataset, optimizer, view_optimizer):
    ...
    #dataset1.aug, dataset1.aug_ratio = args.aug1, 1.0
    ...
    for step, batch in enumerate(tqdm(zip(loader1, loader2, loader3))):
        ...
        x1 = model.forward_cl(batch1.x, batch1.edge_index, batch1.edge_attr, batch1.batch, data_imp)

        data_aug_imp = gen.explain(batch2)
        x2 = model.forward_cl(batch2.x, batch2.edge_index, batch2.edge_attr, batch2.batch, data_aug_imp)

        data_cp_imp = gen.explain(batch3)
        x3 = model.forward_cl(batch3.x, batch3.edge_index, batch3.edge_attr, batch3.batch, data_cp_imp)
...
def main():
    ...
    parser.add_argument('--save_model_path', type=str, default="./lamda_sgcl/sgcl_")

    parser.add_argument('--loss_temp', type=float, default=0.2)
    ...
    #set up Lipschitz Generator
    gen = Generator(args).to(device)
    view_optimizer = torch.optim.Adam(gen.parameters(),lr=args.lr,weight_decay=0.0)

    for epoch in range(1, args.epochs + 1):
        print("====epoch " + str(epoch))
    
        train_loss, ra_loss, cp_loss = train(args, model, gen, device, dataset, optimizer, view_optimizer)
